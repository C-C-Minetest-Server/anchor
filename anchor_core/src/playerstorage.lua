...
function _aps.get_list(player)
	...
	local storagelist = core.deserialize(storageval)
	if not storagelist then
		storageval = meta:get_string("anchor_list_alt")
		storagelist = core.deserialize(storageval)
		if not storagelist then
			anchor.log("warning",
				string.format("The anchor list of player %s is corrupted. Returned a empty list.", player:get_player_name()))
			return {}
		end
	end
	...
	for i,v in ipairs(storagelist) do
		...
		local pos_string = core.pos_to_string({x=v.x,y=v.y,z=v.z})
	...
	if #subject_del > 0 then
		...
		local n_storageval = core.serialize(storagelist)
...
function _aps.add_item(player,item)
	...
	if storageval ~= "" then
		...
		if not storagelist then
			...
			if not storagelist then
				anchor.log("warning",
					string.format("The anchor list of player %s is corrupted. Using a empty list.", player:get_player_name()))
			end
		end
	end
	if not storagelist then storagelist = {} end

	for _, v in ipairs(storagelist) do -- check duplication
	...
	if not item.uuid then
		...
		if not uuid or uuid == "" then
			error("Failed to get the UUID of node " .. core.string_to_pos(pos) .. ". Please pass uuid into add_item.")
		end
		item.uuid = uuid
	end
